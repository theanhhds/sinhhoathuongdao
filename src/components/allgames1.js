...
import { faSpinner, faSearch, faCheckSquare, faAngleUp, faAngleDown, faChild, faBaby, faMale } from '@fortawesome/free-solid-svg-icons';
...
class AllGames extends React.Component{
	constructor(){
		...
		this.handleGameOpenE = this.handleGameOpenE.bind(this);
		this.handleGameOpenL = this.handleGameOpenL.bind(this);
	...
	render(){
		...
		games = this.state.games.map((i, index) => {
			if (1){		//If game has been checked
				...
				cond = (this.state.f_soluong == "" || this.state.f_soluong == i.so_luong) && 
							(this.state.f_doihinh == "" || this.state.f_doihinh == i.doi_hinh) && (this.state.f_child == "" || this.state.f_child == i.child);	
				...
				let symbols;
				if (i.child == 1)
					symbols = 	<div className="w3-large">
									<FontAwesomeIcon title="Dưới 6 tuổi" icon={faBaby} color="blue" size="lg"/> &nbsp;
									<FontAwesomeIcon title="Trên 6 tuổi" icon={faChild} color="orange" size="lg"/> &nbsp;
									<FontAwesomeIcon title="Trên 12 tuổi" icon={faMale} color="green" size="lg"/> &nbsp;
								</div>;
				else if (i.child == 2)
					symbols =   <div className="w3-large">
									<FontAwesomeIcon title="Trên 6 tuổi" icon={faChild} color="orange" size="lg"/> &nbsp;
				...
				if (cond)
					return(
						<div >
							<div className="w3-card w3-display-container w3-animate-bottom">
								<div className={pColor} id={index} onMouseEnter={this.handleGameOpenE} 
									onMouseLeave={this.handleGameOpenL}>
									<div className="w3-container" >
										...
										<div className="w3-row-padding">
											<div className="w3-center w3-col s5 w3-text-red"><i>{so_luong}</i></div>
											<div className="w3-center w3-col s2">{symbols}</div>
											<div className="w3-center w3-col s5 w3-text-red"><i>{doi_hinh}</i></div>
										</div>
										<div className="w3-center font-comic">{ki_nang}</div>
										<div className={openOrClose}>
											<div className="w3-clear"></div>
											<div className="w3-margin" style={{whiteSpace: "pre-wrap"}} >
												{i.mo_ta}<br/>
		...
		<div>
			...
			<div className="w3-sand w3-card w3-padding-large w3-round-xlarge w3-margin-bottom w3-display-container w3-animate-opacity">
				...
				<div className="w3-row-padding">
					...
					<div className="w3-col l4">
						<select className="w3-select" name="f_child" value={this.state.f_child} 
							onChange={this.handleChange}>
							<option value="0" disabled selected>Độ tuổi</option>
							<option value="A">-- Tất cả --</option>
							<option value="1">Dưới 6 tuổi</option>
							<option value="2">Trên 6 tuổi</option>
							<option value="3">Trên 12 tuổi</option>
						</select>
					</div>
				</div>
